...
{
	public:
		IPv4Generator(const char * init, const size_t initsz, const bool repeat);
		~IPv4Generator();

		static void ipv4_generator(char* state, size_t* statesz, size_t statemaxsz,
			char * next, size_t* nextsz, size_t nextmaxsz,
			int repeat)
	private:
		int shift_to_host(struct sockaddr_in* addr);
		int shift_to_next_host(struct sockaddr_in* addr);
		sockaddr_in init_addr;
		sockaddr_in final_addr;
		bool repeat;
};*/

/**@brief shift addr to first, which is HOST & NOT LOCAL & NOT UNKNONWN
 * @repeat - start from the first whe reach the upper bound*/
inline int shift_to_host(struct sockaddr_in* addr, struct sockaddr_in* faddr, bool repeat)
{
	IPv4Info addr_info;
...
inline int shift_to_next_host(struct sockaddr_in* addr, struct sockaddr_in* faddr, bool repeat)
{
	if(ntohl(addr->sin_addr.s_addr) >= ntohl(faddr->sin_addr.s_addr))
	{
		if(repeat)
